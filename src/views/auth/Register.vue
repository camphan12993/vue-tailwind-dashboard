  ...
  <div class="flex flex-col overflow-y-auto md:flex-row">
    <div class="h-32 md:h-auto md:w-1/2">
      <img aria-hidden="true" class="object-cover w-full h-full dark:hidden"
        src="@/assets/imgs/create-account-office.jpeg" alt="Office" />
      <img aria-hidden="true" class="hidden object-cover w-full h-full dark:block"
        src="@/assets/imgs/create-account-office-dark.jpeg" alt="Office" />
    </div>
      ...
      <div class="w-full">
        ...
        <label class="block text-sm">
          ...
          <input
            ...
            :class="{ 'border-red-600': v$.email.$errors.length }" placeholder="Jane Doe" v-model="state.email" />
          <p v-if="v$.email.$errors.length" class="text-red-600 mt-1 text-xs">{{ v$.email.$errors[0].$message }}</p>
        </label>
        <label class="block mt-4 text-sm">
          <span class="text-gray-700 dark:text-gray-400">Password</span>
          <input :class="{ 'border-red-600': v$.password.password.$errors.length }"
            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
            placeholder="***************" type="password" v-model="state.password.password" />
          <p v-if="v$.password.password.$errors.length" class="text-red-600 mt-1 text-xs">{{
              v$.password.password.$errors[0].$message
          }}</p>
        </label>
        <label class="block mt-4 text-sm">
          <span class="text-gray-700 dark:text-gray-400"> Confirm password </span>
          <input :class="{ 'border-red-600': v$.password.confirm.$errors.length }"
            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
            placeholder="***************" type="password" v-model="state.password.confirm" />
          <p v-if="v$.password.confirm.$errors.length" class="text-red-600 mt-1 text-xs">{{
